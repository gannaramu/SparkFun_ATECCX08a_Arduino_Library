...
boolean ATECCX08A::readSignerCertificate(boolean debug)
{
  // read block 0, the first 32 bytes of config zone into inputBuffer
  read(ZONE_DATA_32, ADDRESS_SIGN_CERT_READ_BLOCK_0, 32);

  // copy current contents of inputBuffer into configZone[] (for later viewing/comparing)
  memcpy(&SignerCertificate[0], &inputBuffer[1], 32);

  read(ZONE_DATA_32, ADDRESS_SIGN_CERT_READ_BLOCK_1, 32); // read block 1
  memcpy(&SignerCertificate[32], &inputBuffer[1], 32);   // copy block 1

  read(ZONE_DATA, ADDRESS_SIGN_CERT_READ_BLOCK_2, 4);   // read block 2
  memcpy(&SignerCertificate[64], &inputBuffer[1], 4);   // copy block 2

  read(ZONE_DATA, ADDRESS_SIGN_CERT_READ_BLOCK_3, 4);   // read block 3
  memcpy(&SignerCertificate[68], &inputBuffer[1], 4);   // copy block 3
  ...
  if (debug)
  {
    ...
    for (int i = 0; i < sizeof(configZone); i++)
    {
      ...
      for (int bit = 7; bit >= 0; bit--)
        Serial.print(bitRead(configZone[i], bit)); // print binary WITH preceding '0' bits
      Serial.println();
    }
    Serial.println();
  }
...
boolean ATECCX08A::readSignerPublicKey(boolean debug)
{
  // read block 0, the first 32 bytes of config zone into inputBuffer
  read(ZONE_DATA_32, ADDRESS_SIGN_PUB_KEY_READ_BLOCK_0, 32);

  // copy current contents of inputBuffer into configZone[] (for later viewing/comparing)
  memcpy(&SignerPublicKey[0], &inputBuffer[1], 32);

  read(ZONE_DATA_32, ADDRESS_SIGN_PUB_KEY_READ_BLOCK_1, 32); // read block 1
  memcpy(&SignerPublicKey[32], &inputBuffer[1], 32);         // copy block 1

  read(ZONE_DATA, ADDRESS_SIGN_PUB_KEY_READ_BLOCK_2, 4); // read block 2
  memcpy(&SignerPublicKey[64], &inputBuffer[1], 4);      // copy block 2

  read(ZONE_DATA, ADDRESS_SIGN_PUB_KEY_READ_BLOCK_3, 4); // read block 3
  memcpy(&SignerPublicKey[68], &inputBuffer[1], 4);      // copy block 3

  // pull out serial number from configZone, and copy to public variable within this instance
  memcpy(&SignerPublicKeyX[0], &SignerPublicKey[0], 36); // copy SN<0:3>
  memcpy(&SignerPublicKeyY[0], &SignerPublicKey[36], 36);  // copy SN<4:8>
...
boolean ATECCX08A::sendCommand(uint8_t command_opcode, uint8_t param1, uint16_t param2, uint8_t *data, size_t length_of_data)
{
  ...
  wakeUp();
  // for (int n = 0; n < sizeof(total_transmission); n++)
  // {
  //   char hex_digit[3];
  //   sprintf(hex_digit, "%02X", total_transmission[n]);
...
boolean ATECCX08A::readPublicKey(boolean debug)
{
  ...
  // read block 0, the first 32 bytes of slot10 into inputBuffer
  read(ZONE_DATA, ADDRESS_DATA_READ_SLOT11_BLOCK_0, 32);
  // copy current contents of inputBuffer into storedPublicKey[] (for later viewing/comparing)
  memcpy(&SignerCertificate[0], &inputBuffer[1], 32);

  // read block 0, the first 32 bytes of slot10 into inputBuffer
  read(ZONE_DATA, ADDRESS_DATA_READ_SLOT11_BLOCK_1, 32);
  // copy current contents of inputBuffer into storedPublicKey[] (for later viewing/comparing)
  memcpy(&SignerCertificate[32], &inputBuffer[1], 32);
  ...
  if (debug)
  {
    ...
    {
      Serial.print("0x");
      if ((storedPublicKey[i] >> 4) == 0)
        Serial.print("0"); // print preceeding high nibble if it's zero
      Serial.print(storedPublicKey[i], HEX);
      Serial.print(",");
      if ((sizeof(storedPublicKey) - i) % 16 == 0 && i != 0)
        Serial.println();
    }
    Serial.println();
  }
